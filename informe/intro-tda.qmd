...
Se demuestra en @ConfidenceSetsForPersistenceDiagrams que cuando $\mathcal{M}$ es un *manifold* compacto y suave, bajo condiciones poco restrictivas sobre la distancia de Hausdorff, definida en la @sec-hausdorff-distance, entre la muestra y el *manifold* del que proviene $d_{H}(\mathcal{S}_N, \mathcal{M})$, y una elección apropiada del valor $\epsilon$, entonces el espacio $\mathcal{S}_N^{\epsilon}$ es topológicamente equivalente a $\mathcal{M}$ [@Niyogi2008]. Esto significa que podemos inferir la topología de $\mathcal{M}$ a partir de $\mathcal{S}_N^{\epsilon}$.
