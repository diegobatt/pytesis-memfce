...
La idea central de la homología se basa en la noción de ciclos y bordes en espacios topológicos. En términos sencillos, un ciclo es una combinación lineal de símplices *(simplicial complex)*, siendo un símplice la generalización a dimensiones mayores de un triángulo, cuyos bordes se anulan, es decir, no tienen "agujeros". Por otro lado, un borde es una combinación lineal de símplices cuyos lados forman los bordes de un ciclo. En esencia, los ciclos representan estructuras "cerradas", mientras que los bordes representan estructuras "abiertas".
...
El problema fundamental que se busca abordar con el Análisis Topológico de Datos (TDA) es inferir la homología de un *manifold* $\mathcal{M}$ a partir de una muestra $\mathcal{S}_N$, obtenida mediante una función de densidad $f$ con soporte en $\mathcal{M}$. Como se mencionó brevemente en la @sec-espacios-metricos, esto se realiza mediante la elección de una medida de distancia. Concretamente, se define el espacio generado por una unión de bolas de radio $\epsilon$ centradas en las muestran de $\mathcal{S}_N$, es decir
...
A partir de la descripción de un espacio métrico, la topología bajo análisis puede comenzar a ser geométricamente descripta mediante complejos simpliciales. Formalmente, sea un conjunto  $X = {x_1, \dots, x_k} \subset R^d$ de puntos afínmente independientes, el símplex $\sigma = [x_1, \dots, x_k]$ generado por $X$ es el casco convexo de $X$. Los puntos de $X$ se llaman "vértices" de $\sigma$ mientras que los símplices generados por los subconjuntos de $X$ se llaman "caras" de $\sigma$ @IntroductionTDA.
Estas definiciones cobran especial interés cuando se vuelcan a un conjunto de datos $\mathcal{S}_N$, donde los puntos de la muestra representan nuestros vértices. A partir de este conjunto de datos y la definición de un espacio métrico $(M, \rho)$, existen múltiples formas de construir un complejo simplicial, dos de las más utilizadas son *Čech* y *Vietoris-Rips*. Para un valor $\epsilon > 0$,
el complejo de *Čech*, denotado como $\mathrm{Cech}_{\epsilon}(\mathcal{S}_N)$, se define como el conjunto de símplices $[x_1, \dots, x_k]$ tales que las $k$ bolas cerradas $B(x_i, \epsilon)$ tienen intersección no nula. Por su parte, El complejo de *Vietoris-Rips*, denotado como $\mathrm{Rips}_{\epsilon}(\mathcal{S}_N)$, es el conjunto de símplices $[x_1, \dots, x_k]$ tales que $d_{\mathcal{S}_N}(x_i, x_j) < \epsilon$ para todo $i, j$. Este complejo resulta de cálculo mucho más eficiente que el de *Čech* ya que solo las distancias de a pares entre los puntos son necesarias, y el mismo cumple la siguiente igualdad [@ConfidenceSetsForPersistenceDiagrams]

$$
\mathrm{Cech}_{\epsilon}(\mathcal{S}_N) \subset \mathrm{Rips}_{\epsilon}(\mathcal{S}_N) \subset \mathrm{Cech}_{\sqrt{2} \epsilon}(\mathcal{S}_N)
$$

Por lo que las garantías teóricas que pueda dar el complejo de *Čech* son comunmente aproximadas con el complejo de *Vietoris-Rips*. En la @fig-cech-vs-rips, entraída de @IntroductionTDA se ilustra la diferencia entre estos dos complejos simpliciales.
