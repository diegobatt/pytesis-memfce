...
ACAAAA ESTABAS AGREGANDO LO DE LA DIMENSION

<!-- A modo de ejemplo, podemos ilustrar esta ventaja de los métodos basados en distancia para un conjunto de datos de mayor diemnsionalidad construido de forma sencilla: Si tomamos el conjunto de datos de la circunferencia y agregamos dimensiones con valores constantes, es decir, si tenemos dos puntos de la circunferencia $x_1 = [0, 1]$ y $x_2 = [1, 0]$, obtenemos muestras en mayores dimensiones como $\hat{x}_1 = [0, 1, \sigma_D^{1}, \dots, \sigma_D^{L}]$ y $\hat{x}_2 = [1, 0, \sigma_D^{1}, \dots, \sigma_D^{L}]$, lo que se obtiene es la misma circunferencia pero embebida en un espacio de dimensión mayor.

```{python}

circle_dims_factory = compose(partial(add_dummy_dimensions, d=1), partial(arc, n=n))
plot_dataset(circle_dims_factory(), title="Circunferencia con D = 3"); plt.show()
```
 -->
